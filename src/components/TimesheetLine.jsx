...
const showToast = (message, type = 'info') => {
    ...
    toast.className = `fixed top-5 right-5 p-4 rounded-md text-white shadow-lg z-[100] ${typeClasses[type] || typeClasses['info']}`;
...
// --- Main TimesheetLine Component ---
export default function TimesheetLine({ onClose, resourceId }) {
    // --- State Management ---
    ...
    // --- Constants ---
    const dayHeaders = ['Mon 09/15', 'Tue 09/16', 'Wed 09/17', 'Thu 09/18', 'Fri 09/19', 'Sat 09/20', 'Sun 09/21'];
    const days = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
    ...
    const fieldApiMap = {
        ...
        rlseNumber: 'purchaseOrderRelease',
    ...
    const getDropdownOptions = (targetField, currentLine) => {
        ...
        for (const field of fieldHierarchy) {
            ...
            if (currentLine[field]) {
                filteredData = filteredData.filter(item => String(item[fieldApiMap[field]]) === String(currentLine[field]));
            } else {
                return [];
            }
        }
        ...
        // Sort options numerically if possible, otherwise alphabetically
        ...
        setLines(currentLines => currentLines.map(line => {
            if (line.id === id) {
                ...
                if (changedFieldIndex !== -1) {
                    ...
                    // Auto-populate subsequent fields if they have only one option
                    ...
                    while (true) {
                        ...
                        if (nextOptions.length === 1) {
                            ...
                            currentField = nextField; // Continue the loop from the newly populated field
                        } else {
                            break; // Stop if there isn't a single unique option
                        }
                    }
                }
                return updatedLine;
    ...
    const handleHourChange = (id, day, value) => {
        ...
        // Prevent updates if validation fails, but don't show toast for every key press
    ...
    const handleSubmit = async () => {
        ...
        for (const line of lines) {
            if (!line.project || !line.poLineNumber) {
                showToast(`Skipping line ${line.id}: Missing required project or PO line info.`, 'warning');
                continue; // Skip lines that are not fully filled out
            }
            const now = new Date().toISOString();
            const timesheetHours = days.map((day, index) => {
                const dateParts = dayHeaders[index].split(' ')[1].split('/');
                const date = `2025-${dateParts[0].padStart(2, '0')}-${dateParts[1].padStart(2, '0')}`;
                return { ts_Date: date, hours: line.hours[day] };
            });
            const payload = {
                ...
                hours: parseFloat(Object.values(line.hours).reduce((s, h) => s + h, 0).toFixed(2)),
                DisplayedName: ""
            };
            try {
                ...
                if (response.ok) {
                    ...
                    showToast(`Error for line ${line.id}: ${errorData.message || 'Submission failed'}`, 'error');
                }
            } catch (error) {
        ...
        onClose(); // Close the modal after processing all lines
    };
    
    // --- Render Logic ---
    if (isLoading) {
        return <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="text-white text-xl">Loading Data...</div></div>;
    }
                    ...
                    <table className="w-full text-sm min-w-[1500px]">
                        ...
                        <tbody className="divide-y divide-gray-200/80 bg-white/50">
                            {lines.map((line, index) => {
                                ...
                                const rowTotal = Object.values(line.hours).reduce((s, h) => s + h, 0).toFixed(2);
                                return (
                                <tr key={line.id} className="hover:bg-slate-50/50">
                                    ...
                                    {days.map(day => <td key={day} className="p-2"><input type="number" step="0.5" value={line.hours[day]} onChange={e => handleHourChange(line.id, day, e.target.value)} className={`w-20 text-right bg-white p-1.5 border border-gray-200 rounded-md shadow-sm ${day === 'sat' || day === 'sun' ? 'bg-gray-100 cursor-not-allowed' : ''}`} disabled={day === 'sat' || day === 'sun'} /></td>)}
