...
export default function InvoiceExport() {
    ...
    const handleUnexport = async (invoice) => {
        ...
        const confirmed = window.confirm(`Are you sure you want to reopen invoice ${invoice.invoiceNumber}?`);
        if (!confirmed) return;
        
        try {
            // Show loading state
            const openButton = document.querySelector(`[data-open-invoice="${invoice.invoiceId || invoice.id}"]`);
            if (openButton) {
            ...
            const response = await fetch(`https://timesheet-subk.onrender.com/api/Invoices/${invoice.invoiceId || invoice.id}`, {
