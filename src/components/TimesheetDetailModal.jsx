...
// // --- SVG Icons ---
// const PlusIcon = ({ className = "h-4 w-4" }) => <svg xmlns="http://www.w.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>;
// const CopyIcon = ({ className = "h-4 w-4" }) => <svg xmlns="http://www.w.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>;
// const TrashIcon = ({ className = "h-4 w-4" }) => <svg xmlns="http://www.w.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>;
...
// // --- Helper function to aggressively clean strings for reliable comparisons ---
...
//             // Clean and de-duplicate purchase order data immediately
//             const uniquePoMap = new Map();
//             poDataArray.forEach(po => {
//                 const cleanedResourceDesc = (po.resourceDesc || []).map(d => normalizeString(d));
//                 const uniqueKey = `${po.wa_Code}|${cleanedResourceDesc.join(',')}`;
//                 if (!uniquePoMap.has(uniqueKey)) {
//                     uniquePoMap.set(uniqueKey, { ...po, resourceDesc: cleanedResourceDesc });
//                 }
//             });
//             const uniquePoData = Array.from(uniquePoMap.values());
//             setPurchaseOrderData(uniquePoData);
...
//                 let fullWorkOrderString = '';
//                 const poEntry = uniquePoData.find(po => po.wa_Code === item.workOrder && po.project?.includes(item.projId));
                
//                 if (poEntry) {
//                     const projectIndex = poEntry.project.indexOf(item.projId);
//                     if (projectIndex > -1 && poEntry.resourceDesc[projectIndex]) {
//                         const correspondingDesc = poEntry.resourceDesc[projectIndex];
//                         fullWorkOrderString = normalizeString(`${poEntry.wa_Code} - ${correspondingDesc}`);
//                     }
//                 }
...
//                     plc: item.plc || '',
//                     payType: item.payType || 'SR',
//                     workOrder: fullWorkOrderString, // workOrder is now guaranteed to be clean
//                     poNumber: item.poNumber || '',
//                     rlseNumber: item.rlseNumber || '',
//                     poLineNumber: item.poLineNumber || '',
//                     hours: hoursData,
...
//         setLines(currentLines => currentLines.map(line => {
//             if (line.id === id) {
//                 const finalValue = fieldName === 'workOrder' ? normalizeString(value) : value;
//                 let updatedLine = { ...line, [fieldName]: finalValue };

//                 if (fieldName === 'workOrder') {
//                     if (!finalValue) { 
//                         return { ...createEmptyLine(id), id: line.id }; 
//                     }
//                     const [waCode, desc] = finalValue.split(' - ');
//                     const selectedWorkOrderData = purchaseOrderData.find(item => 
//                         item.wa_Code === waCode && item.resourceDesc.includes(desc)
//                     );
...
//             const otherLinesTotal = currentLines.filter(line => line.id !== id).reduce((sum, line) => sum + (parseFloat(line.hours[day]) || 0), 0);
...
//             return currentLines.map(line => 
//                 line.id === id 
//                 ? { ...line, hours: { ...line.hours, [day]: currentVal } } 
//                 : line
//             );
//         });
//     };
...
//                 ? { ...line, hours: { mon: 0, tue: 0, wed: 0, thu: 0, fri: 0, sat: 0, sun: 0 } } 
//                 : line
//             ));
//         } else {
//             setLines(currentLines => {
...
//         const linesToCopy = lines.filter(line => selectedLines.has(line.id));
//         // ... (rest of the function is standard and correct)
//     };

//     const handleSave = async () => {
//         // ... (This entire function is complex but standard and appears correct)
//     };

//     const workOrderOptions = useMemo(() => 
//         Array.from(new Map(purchaseOrderData.flatMap(item => (item.resourceDesc || []).map(desc => { 
//             const rawLabel = `${item.wa_Code} - ${desc}`; 
//             const normalizedValue = normalizeString(rawLabel);
//             return [normalizedValue, { value: normalizedValue, label: rawLabel }]; 
//         }))).values()), 
//     [purchaseOrderData]);
...
//         lines.forEach(line => {
//             days.forEach(day => { totals[day] += parseFloat(line.hours[day]) || 0; });
//         });
//         return totals;
//     }, [lines, days]);

//     const grandTotal = useMemo(() => Object.values(dailyTotals).reduce((sum, dayTotal) => sum + dayTotal, 0), [dailyTotals]);

//     if (isLoading) { return <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50"><div className="text-white text-lg">Loading Details...</div></div>; }
...
//     const newLines = linesToCopy.map(line => ({
//         ...line, // Copy all data from the original line, including workOrder and its details
//         
//         // Assign a new unique ID for React to track the new row
//         id: `temp-${nextId.current++}`, 
//         
//         // Reset database IDs since this is a new line to be created
//         hourIds: {} 
//     }));
