...
//   const API_URL = "https://timesheet-subk-latest.onrender.com/api/PurchaseOrders";
...
//       (col) => !["PLC CD", "PLC Desc", "Actions"].includes(col)
//     );
...
//   const handleSaveRecord = async () => {
//     const mandatoryColumns = groupColumns.filter(
//       (col) => !["PLC CD", "PLC Desc", "Actions", "Select"].includes(col)
//     );
...
//     try {
//       // Mock API call - replace with your actual import endpoint
...
//       showToast(
//         `Successfully imported ${selectedRows.length} record(s)!`,
//         "success"
//       );
//       setSelectedRows([]);
//     } catch (error) {
...
//                 className="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded text-sm font-normal shadow transition disabled:bg-gray-300 disabled:cursor-not-allowed"
//               >
//                 Import
//               </button>
//               <button
...
//                           className="border p-2 font-bold text-blue-800 text-center whitespace-nowrap bg-blue-50 cursor-pointer select-none"
//                           onClick={() =>
//                             col !== "Actions" && col !== "Select" && handleSort(col)
//                           }
//                         >
//                           <span>
//                             {col}
//                             {col !== "Actions" && col !== "Select" && getSortIcon(col)}
//                           </span>
//                         </th>
//                       ))}
//                     </tr>
...
export default function GroupsTable() {
  ...
  const handleImportPOInfo = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://timesheet-subk-latest.onrender.com/api/PurchaseOrders/import-excel",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleImportVendorMaster = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://timesheet-subk-latest.onrender.com/api/PurchaseOrders/import-venor-master",
        {
          method: "POST",
          body: formData,
        }
